...
public class AbstractTool<T extends Node> implements EventHandler<MouseEvent>
{
	...
	/**
	 * Constant to check with {@link #bitMask} in constructor
	...
	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseReleased(MouseEvent)} should be triggered on
	 * {@link MouseEvent#MOUSE_RELEASED} events
	 */
	protected static final int RELEASED = 2;

	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseClicked(MouseEvent)} should be triggered on
	...
	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseMoved(MouseEvent)} should be triggered on
	 * {@link MouseEvent#MOUSE_MOVED} events
	 */
	protected static final int MOVED = 8;

	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseDragged(MouseEvent)} should be triggered on
	 * {@link MouseEvent#MOUSE_DRAGGED} events
	 */
	protected static final int DRAGGED = 16;

	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseEntered(MouseEvent)} should be triggered on
	...
	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseExited(MouseEvent)} should be triggered on
	...
	/**
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseEnteredTarget(MouseEvent)} should be triggered on
	 ...
	 * Constant to check with {@link #bitMask} in constructor
	 * to define if {@link #mouseExited(MouseEvent)} should be triggered on
	 * {@link MouseEvent#MOUSE_EXITED_TARGET} events
	 */
	 ...
	 * #mouseReleased
	 * @param capture if true indicate this tool should be registered on the
	 * root as an Event Filter, otherwise it should be registered on the root as
	 * an {@link EventHandler}
	 * @param consume infdicates if events should be consumed after being
	 * processed or not. Events not consumed in the tool bubble up to the parent
	 * node.
	 * @param parentLogger parent logger
	 */
...
//		logger.info(event.toString());
	 ...
	 * @apiNote Even though this method does nothing it can't be abstract
	 * otherwise any sub-class will be required to implement this method which
	 * might not be necessary depending on the events captured by sub-class.
	 */
