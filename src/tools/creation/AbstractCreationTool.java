...
{
	/**
	 * The {@link Figure} to create and draw in {@link AbstractTool}'s root node
	 ...
	 * {@link AbstractDrawingTool}'s drawingModel and removed from
	 * {@link AbstractTool}'s root Pane.
	 ...
	 * tool (will also creates the required number of
	 * {@link AbstractMultiStepDrawingTool}'s tips)
	 */
	 ...
	 * 	<li>Reset {@link AbstractMultiStepDrawingTool}'s currentStep to 0</li>
	 * </ul>
	 */
	 ...
	 * {@link AbstractTool}'s root and {@link #reset()} this tool
	 */
	protected void cancelFigure()
	{
		if (figure != null)
		{
			...
			if (!removed)
			{
				logger.warning("figure " + figure + " not removed from root Pane");
			}
		}
		else
		{
	 ...
	 * <ol>
	 * 	<li>Removing it from {@link AbstractTool}'s root node</li>
	 * 	<li>Asks the #historyManager to create a new record</li>
	 * 	<li>Then adding the figure to the {@link AbstractDrawingTool}'s
	 * 	drawingModel</li>
	...
	protected void terminateFigure()
	{
		...
		if (figure != null)
		{
			...
			// - record Drawing model current state before adding this
			historyManager.record();

			// 	- add it to #drawingModel
			boolean added = drawingModel.add(figure);
			if (!added)
			{
				logger.warning("figure not added to drawing model");
			}
		}
		else
		{
