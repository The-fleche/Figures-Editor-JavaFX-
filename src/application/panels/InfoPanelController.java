 ...
 * events came from the contained {@link javafx.scene.Group} /
 * {@link javafx.scene.shape.Shape}s.
 * When entering a {@link javafx.scene.shape.Shape} the Info Panel described in
 * InfoPanel.fxml and reflected in FXML related attributes should be updated
 * with Data corresponding to the {@link FocusedFigureTool}'s focusedFigure
 * containing the {@link javafx.scene.shape.Shape} entered.
 * When exiting a {@link javafx.scene.shape.Shape} the Info Panel should be
 * cleared of its data.
 * @author davidroussel
 */
...
{
	/**
	 * Extra info label (i.e. in the information bar at the bottom of the app)
	 * where to display the name of the figure under cursor
	 */
	 ...
	 * @param pane as {@link AbstractTool}'s root node
	 ...
	 * @param resources Resource Bundle containing translations resources for the UI (or null)
	 */
	@Override
	 ...
	 * with {@link javafx.event.Event#getTarget()}
	...
	private void fillInfoPanel(Figure figure)
	{
		...
		/*
		 * TODO 801 InfoPanelController#fillInfoPanel
		 * Fill this panel's FXML attributes with data from figure
		 * 	- figure name (String) --> figureNameLabel & infoLabel
		 * 	- figure type (using the FigureIconsFactory) --> figureTypeImageView
		 * 	- fill color --> fillColorCircle
		 * 	- edge color --> fillColorCircle
		 * 	- line type --> lineTypeImageView
		 * 	- top left --> topLeftXLabel & topLeftYLabel
		 * 	- bottom right --> bottomRightXLabel & bottomRightYLabel
		 * 	- width --> widthLabel
		 * 	- height --> heightLabel
		 * 	- center --> centerXLabel & centerYLabel
		 * 	- Root's translate x --> translationXLabel
		 * 	- Root's translate y --> translationYLabel
		 * 	- Root's rotate --> rotationLabel
		 * 	- Root's scale --> scaleLabel
		 */
		logger.info("Over figure " + figure);
	}
	...
	{
		/*
		 * TODO 802 InfoPanelController#clearInfoPanel
		 * Clears this panel's FXML attributes content
		 * 	- figureNameLabel
		 * 	- figureTypeImageView
		 * 	- fillColorCircle
		 * 	- edgeColorCircle
	...
	/**
	 * Sets the extra Info Label where to display the name of the figure under
	 * cursor
	 * @param infoLabel the infoLabel to set (or null to unset it)
	 */
